    ...
    def extract_transactions(self):
        with open(self.transaction_source_document, newline='', encoding='utf-8') as f:
            ...
            if not headers or "Transaktionscode" not in headers:
                self.logger.error(f"Headers missing or 'Transaktionscode' not found in {self.transaction_source_document}.")
                return []
            for row in reader:
                try:
                    transaction.transaction_date = datetime.strptime(row.get('\ufeff"Datum"', ""), "%d.%m.%Y").strftime("%Y-%m-%d").strip()
                except Exception as e:
                    self.logger.error(f"Date conversion error in {self.transaction_source_document}: {e}")
                ...
                transaction.transaction_source_document   = self.transaction_source_document
