...
class BarclaysPDFExtractor(BasePDFExtractor):
    def create_transaction(self, iso_date, description, amount_str, sender, currency, invoice, to_field):
        try:
            ...
            return Transaction(iso_date, description, amount_value, sender, self.transaction_source_document, self.bank_type, currency, invoice, to_field)
        except Exception as e:
            self.logger.error(f"Error converting amount '{amount_str}' in {iso_date} for file {self.transaction_source_document}.")
    ...
    def extract_transactions(self):
        with pdfplumber.open(self.transaction_source_document) as pdf:
            if not pdf.pages:
                self.logger.warning(f"No pages found in {self.transaction_source_document}")
                return []
                ...
                for line in lines:
                    ...
                    if match:
                        ...
                        try:
                            ...
                            self.logger.error(f"Error converting number string '{num_str}' in file {self.transaction_source_document}: {e}")
                            value = 0.0
                        if sign == "-":
                            value = -value
                        try:
                            ...
                            self.logger.error(f"Date conversion error for '{date}' in file {self.transaction_source_document}: {e}")
                            iso_date = date
                        sender = self.account
                        currency = "EUR"
                        invoice = ""
